...
int main(int argc, char **argv) {
    ...
    if(argc != 7){
        std::cerr << "Usage: " << argv[0] << " <queue_name> <producers> <size_queue> <items> <rand_center> <rand_amplitude>" << std::endl;
        return 1;
    }
    ...
    Queues::foreach([&]<template <typename> typename Q>() { //captures all the outer scope
        ...
        if (name == queueName) {
            ...
            "DEBUG: " <<
#endif
            benchmark<Q>(producers,1, sizeQueue, duration, center, amplitude) << std::endl;
...
#endif
        //Assumes consumer doesn't do any random work
